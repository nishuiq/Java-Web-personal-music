...
public class MusicUpdateServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        int result = DaoFactory.getMusicDaoInstance().update(music);
        PrintWriter writer = resp.getWriter();

        Gson gson = new Gson();
        if (result == 0) {
            String json = gson.toJson("{\"status\":false, \"msg\":\"修改失败！检查该歌曲是否已存在。\"}");
            writer.write(json);
            return;
        }

        String json = gson.toJson("{\"status\":true, \"msg\":\"修改数据成功！\"}");
        writer.write(json);
